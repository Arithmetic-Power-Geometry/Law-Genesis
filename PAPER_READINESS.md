...
The manuscript should begin only after the following gates are satisfied. This keeps the paper theory-first and prevents the narrative from moving ahead of the mathematics.

## Gate 1 — Frozen mathematical core

Required:

- Common-Law Congruence Representation theorem fixed.
- Law Genesis Cost definition fixed.
- Forced Abstraction and critical seed rank definitions fixed.
...
- explicit nonlinear multi-law witness;
- exact regression test;
- matched low-order observables before separation;
- at least one clock-free intrinsic witness.

Status: satisfied by the five-state raw-disagreement theorem and the four-state permutation witness.
...
- per-state microscopic-output multiplicities;
...
- one-command reproduction passes;
- all headline witnesses frozen as explicit source files;
- generated JSON/CSV results available;
